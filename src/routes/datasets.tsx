  ...
  return (
    <div className="mb-4">
      <button
        ...
        className="flex items-center gap-1.5 text-[12px] font-semibold text-ink mb-1.5 w-full"
      >
        <ChevronRight
...
function Dropdown({ label, options }: { label: string; options: string[] }) {
  ...
  return (
    <div className="relative">
      <div className="text-[11px] uppercase tracking-[0.1em] text-ink-3 mb-1.5 font-medium">
        {label}
      </div>
      <button
        onClick={() => setOpen(!open)}
...
const initialPipeline: Step[] = [
  { id: "s1", kind: "from", parts: [{ label: "FROM", value: "Dataset_A.csv", mono: true }] },
  {
    id: "s2",
    kind: "join",
    parts: [
...
function optionsForPart(step: Step, partLabel: string, ctx: EditCtx): PartOptions {
  ...
  switch (step.kind) {
    ...
    case "join":
      if (partLabel === "JOIN") {
        const fromName = ctx.steps
          .find((s) => s.kind === "from")
          ?.parts.find((p) => p.label === "FROM")?.value;
    ...
    case "aggregate":
      if (partLabel === "AGGREGATE")
        return {
          kind: "list",
          options: columnsFor(refs, ctx.schemaBySlot, (a) => a.type === "num"),
        };
      if (partLabel === "BY") return { kind: "list", options: aggOptions };
      break;
    case "filter":
      if (partLabel === "FILTER")
        return { kind: "list", options: columnsFor(refs, ctx.schemaBySlot) };
      return { kind: "text" };
    case "sort":
      if (partLabel === "SORT")
        return { kind: "list", options: columnsFor(refs, ctx.schemaBySlot) };
  ...
  return (
    ...
    >
        ...
        return (
          <span key={i} className="inline-flex items-center gap-1.5">
            {isFilterOp ? (
              ...
              <span className="text-[10.5px] uppercase tracking-[0.1em] font-semibold text-ink-3">
                {p.label}
              </span>
            )}
            <EditablePart
      ...
      {step.kind !== "from" && (
        <button
          onClick={onRemove}
          className="ml-1 h-6 w-6 rounded-md flex items-center justify-center text-ink-3 opacity-0 group-hover:opacity-100 hover:bg-surface-hover hover:text-ink transition"
  ...
  return (
    <div className="px-5 pt-4 pb-3 border-b border-hairline">
      <div className="flex items-center justify-between mb-2">
        <div className="text-[10.5px] uppercase tracking-[0.1em] font-semibold text-ink-2">
          Pipeline
        </div>
      ...
      {!hasOps ? (
        <p className="text-[13px] text-ink-2 italic">
          Configure the controls below to see your transformation summarised here.
        </p>
      ) : view === "compact" ? (
        <div className="flex flex-wrap items-center gap-x-1.5 gap-y-2">
          {steps.map((s, i) => {
            ...
            const showArrow =
              i > 0 && s.kind !== "join" && prev?.kind !== "from"
                ? true
                : i > 0 && s.kind !== "join" && prev?.kind === "from"
                  ? false
                  : false;
            return (
              <span key={s.id} className="inline-flex items-center gap-1.5">
                {showArrow && (
                  <ArrowRight className="h-3.5 w-3.5 text-coral shrink-0" strokeWidth={2.25} />
                )}
                <SentenceFragment
                  step={s}
                  ctx={ctx}
        ...
        <div className="flex flex-col gap-1.5">
          {steps.map((s) => (
            <div
              key={s.id}
              className="grid grid-cols-[80px_1fr] items-center gap-3 px-2 py-1.5 rounded-md hover:bg-canvas/60"
            >
              <span className="text-[10.5px] uppercase tracking-[0.1em] font-semibold text-ink-2">
                {s.kind}
              </span>
...
function PipelineStrip({
    ...
    setSteps((prev) =>
      prev.map((s) =>
        s.id === stepId
          ? {
              ...s,
              parts: s.parts.map((p, i) => (i === partIndex ? { ...p, label: nextLabel } : p)),
            }
          : s,
      ),
    );
  ...
  const addStep = (kind: StepKind) => {
    ...
    const templates: Record<Exclude<StepKind, "from">, Step["parts"]> = {
      join: [
        { label: "JOIN", value: otherSlot, mono: true },
        { label: "ON", value: idCol, mono: true },
        { label: "USING", value: "Inner Join" },
      ],
      aggregate: [
        { label: "AGGREGATE", value: numCols[0] ?? "", mono: true },
        { label: "BY", value: "Mean" },
      ],
      filter: [
        { label: "FILTER", value: numCols[0] ?? allCols[0] ?? "", mono: true },
        { label: "Greater than", value: "0", mono: true },
      ],
      sort: [
        { label: "SORT", value: allCols[0] ?? "", mono: true },
    ...
    <>
      ...
      <div className="border-b border-hairline px-5 py-4">
        <div className="flex items-center gap-1.5 mb-2">
          ...
          <span className="text-[10.5px] text-ink-3/70 tabular">
        ...
        <div className="flex flex-wrap items-center gap-2">
          {steps.map((s, i) => (
            <span key={s.id} className="inline-flex items-center gap-2">
              ...
              />
              {i < steps.length - 1 && (
                <ArrowRight className="h-3.5 w-3.5 text-ink-3/60 shrink-0" strokeWidth={2} />
              )}
            </span>
          ))}
            ...
            {!adding ? (
              ...
              <div className="inline-flex items-center gap-1 h-9 px-1.5 rounded-md border border-hairline bg-surface shadow-[var(--shadow-sm)]">
                ...
                <button
                  ...
                  className="h-7 w-7 rounded-[5px] flex items-center justify-center text-ink-3 hover:bg-surface-hover transition"
                  aria-label="Cancel"
                >
                  <X className="h-3 w-3" />
                </button>
...
function PreviewTable({ result }: { result: RunResult }) {
          ...
          {EXPORT_FORMATS.map((f) => (
            <button
              ...
              className={`block w-full text-left px-3 py-2 text-[12.5px] transition ${
                f.disabled ? "text-ink-3/50 cursor-not-allowed" : "text-ink hover:bg-surface-hover"
              }`}
            >
              {f.label}
...
function DatasetsPage() {
  ...
  const [datasetSlots, setDatasetSlots] = useState<string[]>(project ? project.datasets : []);
        ...
        if (prev[slot]?.size) next[slot] = prev[slot];
          ...
          const attrs =
            slot === "Dataset_A.csv"
              ...
              : slot === "Dataset_B.csv"
                ? datasetB
                : (importedDatasets[slot]?.attrs ?? apiSchemaBySlot[slot] ?? []);
  ...
  const saveSummary = (
    <>
      ...
      {isPreviewFetching && (
        <span
          className="ml-2 inline-block h-1.5 w-1.5 rounded-full bg-coral/70 animate-pulse align-middle"
          aria-label="Loading"
        />
      )}
      ...
      <div className="flex flex-col lg:flex-row gap-5 min-w-0">
        ...
        <aside className="w-full lg:w-[280px] shrink-0 bg-surface rounded-xl border border-hairline shadow-[var(--shadow-card)] p-4 lg:self-start lg:sticky lg:top-[72px] max-h-[min(280px,40vh)] lg:max-h-[calc(100vh-90px)] overflow-y-auto">
          <div className="flex items-center justify-between mb-3">
            <h2 className="text-[13px] font-semibold text-ink uppercase tracking-[0.08em]">
              Attributes
            </h2>
          ...
          <div className="mt-3 pt-3 border-t border-hairline flex items-center gap-3 text-[10.5px] text-ink-2">
            <span className="flex items-center gap-1">
              <Key className="h-2.5 w-2.5 text-data-plum" strokeWidth={2.5} />
              ID
            </span>
            <span className="flex items-center gap-1">
              <Hash className="h-2.5 w-2.5 text-data-slate" strokeWidth={2.5} />
              Numeric
            </span>
            <span className="flex items-center gap-1">
              <Type className="h-2.5 w-2.5 text-data-sage" strokeWidth={2.5} />
              Category
            </span>
          </div>
        </aside>
